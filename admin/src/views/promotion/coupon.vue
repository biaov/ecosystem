<template>
  <c-layout-list title="优惠券">
    ...
    <template #extra>
      <a-button type="primary" :href="$formatter.publicURL('/promotion/coupon-add')" v-perm="permKey.create">新增</a-button>
    </template>
    <template #list>
      <a-table :data-source="data.items" row-key="id" :loading="loading" :pagination="$formatter.pagination(data)" @change="setPage">
        ...
        <a-table-column title="操作" :width="180">
          <template #="{ record }">
            <a-button type="link" size="small" :href="$formatter.publicURL(`/promotion/coupon-data/${record.id}`)" v-perm="permKey.list">数据</a-button>
            <a-button type="link" size="small" :href="$formatter.publicURL(`/promotion/coupon-edit/${record.id}`)" v-perm="permKey.update">编辑</a-button>
