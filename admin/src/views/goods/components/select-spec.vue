<template>
  ...
  <a-space direction="vertical" :size="20">
    <a-space wrap v-for="(item, index) in specsModel" :key="index" class="w-full">
      <a-input v-model:value="item.sku" placeholder="SKU" :disabled="disabled || isEdit(item.id)" />
      ...
      <a-button type="text" @click="onDelete(index)" :disabled="disabled || specsModel.length <= 1 || isEdit(item.id)">
        <template #icon>
          <c-ant-icon name="CloseCircleFilled" :color="specsModel.length > 1 && !isEdit(item.id) ? '#f56c6c' : 'rgba(0, 0, 0, 0.2)'" />
        </template>
      </a-button>
    </a-space>
...
const isEdit = computed(() => (id?: number) => !!(props.disabledSku && id))
const initSpec = () => {
