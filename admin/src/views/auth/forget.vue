  ...
  <div class="w-screen h-screen bg-radial flex justify-center items-center relative">
    ...
    <a-card title="忘记密码" class="w-340">
      <a-form>
        ...
        <a-form-item>
          <a-input-password v-model:value="formState.password" placeholder="请输入新的密码" :maxlength="32" />
        </a-form-item>
        <a-form-item>
          <a-input-password v-model:value="formState.cpassword" placeholder="请输入确认密码" :maxlength="32" />
        </a-form-item>
        <a-form-item>
          <a-button type="primary" block @click="handleSubmit">重置密码</a-button>
        </a-form-item>
      </a-form>
      <router-link to="/login" class="text-center text-info">返回登录</router-link>
    </a-card>
  </div>
</template>
...
const handleSubmit = async () => {
  ...
  await forgetApi.post({ ...formState.value, source: sourceEnum.admin })
