 ...
 */
export const setStorage = (key: string, value: unknown, type?: StorageType) => {
  const newType = type || StorageTypeValue.Local
  switch (newType) {
 ...
 * @returns { unknown } 缓存值
 */
export const getStorage = (key: string, type?: StorageType): unknown => {
  const newType = type || StorageTypeValue.Local
  switch (newType) {
    case StorageTypeValue.Local:
 ...
 * @param { StorageType } type 缓存类型
 */
export const removeStorage = (key: string, type?: StorageType) => {
  const newType = type || StorageTypeValue.Local
  switch (newType) {
    case StorageTypeValue.Local:
